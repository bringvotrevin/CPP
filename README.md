...
reinterpret_cast(타입캐스트)를 사용해보는 과제. 임의의 포인터 타입끼리의 변환과 정수형을 포인터로 바꾸는 것을 허용하는 캐스트 연산자이다. 포인터를 정수 표현으로 변환할 때 유용하게 사용. 다른 형태의 포인터로 변경이 가능하기 때문에 잘못 사용하는 경우 결과 값이 컴파일러에 따라 다르거나 예상치 못한 값이 나올 수도 있음</br>
Data 클래스 포인터를 uintptret_t타입 변수에 담고 그걸 다시 Data 클래스 포인터로 변환해보며 그 과정에서 주소값이 똑같이 담기는지, 클래스 안에 있는 값은 변함 없이 잘 보존되어 있는지를 확인
